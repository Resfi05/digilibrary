...
<head>
    <meta charset="UTF-8">
    <title>Laporan Buku - DigiLibrary</title>
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .lap-header { background:linear-gradient(135deg,#7c3aed,#1a56db);padding:28px 36px;color:white;position:relative;overflow:hidden; }
...
<div class="laporan-wrap">
    <div class="lap-header">
        ...
        <div class="lh-title">LAPORAN DATA KOLEKSI BUKU</div>
        <div class="lh-sub">
            Total Koleksi: {{ $books->count() }} buku
    ...
    <div class="lap-body">
        ...
        <table class="lap-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $i+1 }}</td>
                    <td style="font-weight:600;color:#111827">{{ $b->judul }}</td>
                    <td>{{ $b->penulis }}</td>
                    <td style="font-size:.72rem">{{ $b->penerbit ?? '-' }}</td>
                    <td style="font-size:.72rem">{{ $b->category->nama_kategori ?? '-' }}</td>
                    <td>{{ $b->tahun_terbit ?? '-' }}</td>
                    <td class="{{ $b->stok > 3 ? 'stok-ok' : ($b->stok > 0 ? 'stok-low' : 'stok-empty') }}">
                        {{ $b->stok }}
                    </td>
                    <td style="font-weight:700;color:#1a56db">{{ $b->peminjaman_count }}x</td>
                    <td style="font-size:.72rem;color:#94a3b8">{{ $b->isbn ?? '-' }}</td>
                </tr>
                @empty
