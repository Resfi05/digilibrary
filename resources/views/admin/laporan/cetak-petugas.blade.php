...
<head>
    <meta charset="UTF-8">
    <title>Laporan Petugas - DigiLibrary</title>
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .lap-header { background:linear-gradient(135deg,#f59e0b,#ef4444);padding:28px 36px;color:white;position:relative;overflow:hidden; }
...
<div class="laporan-wrap">
    <div class="lap-header">
        ...
        <div class="lh-title">LAPORAN DATA PETUGAS PERPUSTAKAAN</div>
        <div class="lh-sub">
            Total Petugas: {{ $petugas->count() }} orang
    ...
    <div class="lap-body">
        <div class="section-title">Daftar Petugas</div>
        <table class="lap-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:.72rem">{{ $p->email }}</td>
                    <td>{{ $p->phone ?? '-' }}</td>
                    <td style="font-size:.72rem">{{ $p->address ?? '-' }}</td>
                    <td>
                        <span class="{{ $p->is_active ? 'status-a':'status-n' }}">
                            {{ $p->is_active ? 'Aktif':'Nonaktif' }}
                        </span>
                    </td>
                    <td>{{ $p->created_at?->format('d M Y') }}</td>
                    <td style="font-size:.72rem;color:#94a3b8">DL-{{ str_pad($p->id,6,'0',STR_PAD_LEFT) }}</td>
                </tr>
                @empty
