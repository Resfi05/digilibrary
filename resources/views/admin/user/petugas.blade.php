...
@section('page-sub', 'Kelola semua akun petugas perpustakaan')
...
<div class="breadcrumb">
    ...
    <span>›</span><span>Daftar Petugas</span>
</div>

{{-- STATS --}}
<div class="user-stats">
    <div class="user-stat-card">
        <div class="user-stat-icon" style="background:#ede9fe">👨‍💼</div>
        <div>
            <div class="user-stat-num">{{ number_format($stats['total']) }}</div>
            <div class="user-stat-label">Total Petugas</div>
        </div>
    </div>
    <div class="user-stat-card">
...
<div style="display:grid;grid-template-columns:1fr 260px;gap:20px">
    ...
    <div class="adm-card">
        <div class="filter-bar">
            <form action="{{ route('admin.petugas.list') }}" method="GET"
                ...
                <a href="{{ route('admin.petugas.list') }}" class="filter-btn">Reset</a>
                @endif
            </form>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="tbl-actions">
                                ...
                                <form action="{{ route('admin.petugas.list.toggle', $user->id) }}" method="POST" style="display:inline">
                                    @csrf @method('PUT')
                                ...
                                <form action="{{ route('admin.petugas.list.destroy', $user->id) }}" method="POST" style="display:inline">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="tbl-btn tbl-btn-del" title="Hapus"
                                        onclick="return confirm('Hapus petugas {{ addslashes($user->name) }}?')">
                                        🗑
                                    </button>
                                </form>
                            </div>
    ...
    <div>
        ...
        <div class="rs-card">
            ...
            <div class="info-box">
                ℹ️ Petugas dengan status nonaktif tidak dapat login. Aktifkan kembali kapan saja dengan tombol 🔓.
            </div>
        </div>
        <div class="rs-card">
...
function editUser(id, name, email) {
    document.getElementById('editForm').action = '/admin/petugas-list/' + id;
